        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-1">
              ...
              {/* Profile Link - Always visible when user is logged in */}
              {user && (
                <Link
                  to="/profile"
                  className="px-4 py-2.5 text-sm font-semibold text-purple-600 hover:bg-purple-50 rounded-xl transition-all duration-200 hover:scale-105"
                >
                  Profile
                </Link>
              )}
              {/* Review Profile Form Button */}
              <Button
                variant="outline"
                size="sm"
                onClick={() => setShowProfileModal(true)}
                className="px-4 py-2.5 text-sm font-semibold text-blue-600 border-blue-200 hover:bg-blue-50 rounded-xl transition-all duration-200 hover:scale-105"
              >
                Review Profile Form
              </Button>
          ...
          <div className="md:hidden bg-white/95 backdrop-blur-md border-t border-gray-100 shadow-lg">
            <div className="px-4 py-3 space-y-1">
              ...
              {user && (
                <Link
                  to="/profile"
                  className="block px-4 py-3 text-sm font-semibold text-purple-600 hover:bg-purple-50 rounded-xl transition-all duration-200"
                  onClick={() => setMenuOpen(false)}
                >
                  Profile
                </Link>
              )}
              {/* Review Profile Form Button in Mobile Menu */}
              <button
                onClick={() => {
                  setShowProfileModal(true);
                  setMenuOpen(false);
                }}
                className="block w-full text-left px-4 py-3 text-sm font-semibold text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-200"
              >
                Review Profile Form
              </button>
              {profile?.is_moderator && (
